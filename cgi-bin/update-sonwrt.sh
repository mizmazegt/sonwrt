...
OUT_ZIP="/tmp/sonwrt.zip"
WORKDIR="/tmp/sonwrt_update"
...
if ! grep -q "config uhttpd 'sonwrt'" "$UHTTPD_CONF"; then
cat >> "$UHTTPD_CONF" <<EOF

config uhttpd 'sonwrt'
    option listen_http '0.0.0.0:2222'
    option home '/www/sonwrt'
